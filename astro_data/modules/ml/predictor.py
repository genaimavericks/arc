...
class Predictor:
    ...
    def _load_models(self):
        ...
        for model_name, file_path in model_files.items():
            try:
                ...
                # Instead of exiting, we'll use custom prediction methods
    ...
    def predict(self, model_name: str, input_data: pd.DataFrame) -> pd.DataFrame:
        ...
        # If the model is None (failed to load), use custom prediction directly
        if self.models[model_name] is None:
            print(f"Using custom prediction for {model_name} (model was not loaded)")
            return self._use_custom_prediction(model_name, input_data)
            
        try:
            # First try the standard prediction method
    ...
    def _use_custom_prediction(self, model_name: str, input_data: pd.DataFrame):
        """Helper method to route to the appropriate custom prediction method"""
