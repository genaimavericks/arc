...
# Lightening Application Packager
# This script creates a deployable package of the Lightening application

# Exit on any error
set -e

echo "=== Lightening Application Packager ==="
...
window.__LIGHTENING_CONFIG__ = {
  apiBaseUrl: "http://172.105.77.119:19090"
};
EOF
  ...
  if [ -f "$htmlfile" ]; then
    ...
    if ! grep -q "config.js" "$htmlfile"; then
      ...
      if [[ "$(uname)" == "Darwin" ]]; then
        ...
        # Linux and Windows (Git Bash/WSL) don't need the empty string
        sed -i 's/<script/<script src="\/config.js"><\/script>\n<script/' "$htmlfile"
      fi
    fi
  fi
...
echo "Lightening application is ready to start!"
...
export DB_NAME=${DB_NAME:-lightening}

# Start the application
echo "Starting Lightening application..."
source .venv/bin/activate
...
tar -czf lightening-deployment.tar.gz package/

echo "=== Package created successfully ==="
echo "Deployment archive: lightening-deployment.tar.gz"
