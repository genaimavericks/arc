...
def initialize_churn_system():
    ...
    try:
        # Import directly from the api.churn_astro package
        try:
            from api.churn_astro.page.churn_bot import parse_query_to_input as pqi
            from api.churn_astro.page.churn_prediction import make_predictions as mp
            from api.churn_astro.utils.data_processing import clean_and_encode_data_for_7 as cedf
            
            # If we get here, all imports succeeded
            parse_query_to_input = pqi
            make_predictions = mp
            clean_and_encode_data_for_7 = cedf
            print("Successfully imported from api.churn_astro package")
        except ImportError as e:
            print(f"Error importing from api.churn_astro package: {e}")
            ...
            try:
                ...
                # Import churn_bot.py
                bot_path = Path(__file__).parent / "churn_astro" / "page" / "churn_bot.py"
                if bot_path.exists():
                ...
                pred_path = Path(__file__).parent / "churn_astro" / "page" / "churn_prediction.py"
                if pred_path.exists():
                ...
                proc_path = Path(__file__).parent / "churn_astro" / "utils" / "data_processing.py"
                if proc_path.exists():
