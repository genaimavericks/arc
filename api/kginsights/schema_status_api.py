...
def get_neo4j_stats(schema_id: int, db: Session):
    ...
    try:
        ...
        if not graph_config:
            ...
            raise Exception(f"No configuration found for graph {graph_name} or default_graph")
        
        # Connect to Neo4j
        connection_params = parse_connection_params(graph_config)
        if not connection_params:
            raise Exception("Could not get Neo4j connection parameters")
        ...
        if not all([uri, username, password]):
            raise Exception(f"Missing required Neo4j connection parameters: uri={uri}, username={username}, password={'*' * len(password) if password else None}")
