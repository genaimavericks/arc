...
# Assuming LLMProvider is accessible, adjust the import path as needed
# If llm_provider.py is in /test/api/kgdatainsights, we might need to adjust PYTHONPATH
# or move/copy LLMProvider to a shared location like /api/utils.
# For now, let's assume it can be imported relative to the project root somehow,
# or adjust the import path based on your project structure.
# A simple approach for now might be a relative path if running scripts from a common root.
# If running this file directly, Python path adjustments might be needed.
# Now that LLMProvider is in the same directory, use a relative import.
try:
    # If LLMProvider was moved to utils
    from .llm_provider import LLMProvider 
    from langchain_core.language_models.chat_models import BaseChatModel
except ImportError as e:
    print(f"Error importing LLMProvider or BaseChatModel: {e}. Ensure llm_provider.py is accessible.")
    LLMProvider = None
    BaseChatModel = None
...
class CsvToCypherGenerator:
    ...
    def __init__(self, schema_path: str, csv_path: str, provider_name: str, model_name: Optional[str] = None):
        """
        Initializes the generator.

        Args:
            schema_path: Path to the JSON graph schema file.
        ...
        """
        if not LLMProvider:
            raise ImportError("LLMProvider could not be imported. Cannot proceed.")
    ...
    def _load_schema(self) -> Optional[Dict[str, Any]]:
        """Loads the graph schema from the JSON file."""
        try:
            ...
            print(f"Error: Could not decode JSON from {self.schema_path}")
            return None
        except Exception as e:
            print(f"An unexpected error occurred loading schema: {e}")
            return None
    ...
    def _select_rows(self) -> List[Dict[str, Any]]:
        ...
        # Middle data row (approximate middle)
        if num_rows >= 3:
             # Avoid selecting index 1 if it's also the middle in a 3-row df
            middle_index = (num_rows -1) // 2 # 0-based index for middle row
            # Ensure middle index is not the same as the first selected index (1)
            # Adjust if necessary, e.g., pick middle_index + 1 if it's 1 and num_rows > 2
            # However, a simpler approach is just to add it if it's not already selected
            if middle_index not in indices_to_select:
                 indices_to_select.append(middle_index)
            elif middle_index + 1 < num_rows and (middle_index + 1) not in indices_to_select:
                 # Try the next one if middle was already picked and there's room
                 indices_to_select.append(middle_index + 1)
...
if __name__ == '__main__':
    ...
    # Assuming schema.json is in ../test/api/kgdatainsights and csv is in ../api/uploads
    # Running from rsw/api/utils -> ../../test/api/kgdatainsights/schema.json
    # Running from rsw/api/utils -> ../../api/uploads/foamfactory_1000.csv
    # Or better: Define paths relative to the project root if possible
    project_root = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))) # Get path to 'rsw' dir
    schema_file = os.path.join(project_root, 'test', 'api', 'kgdatainsights', 'schema.json')
    ...
    if not os.path.exists(schema_file):
         print(f"Error: Schema file not found at calculated path: {schema_file}")
        ...
        if not os.getenv(key_needed):
            ...
            try:
                generator = CsvToCypherGenerator(schema_file, csv_file, provider, model)
