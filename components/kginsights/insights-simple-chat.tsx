      ...
      switch (type) {
        ...
        case 'autocomplete_suggestions':
          ...
          setAutocompleteSuggestions(
            content.suggestions.map((text: string) => ({ text }))
          )
          setShowAutocomplete(content.suggestions.length > 0)
          break
        ...
        <div className="flex gap-2 relative">
          <div className="relative flex-1">
            ...
            {showAutocomplete && autocompleteSuggestions.length > 0 && (
              <div className="absolute z-10 bg-card border rounded-md shadow-md w-64 max-h-48 overflow-y-auto">
                {autocompleteSuggestions.map((suggestion, index) => (
                  <div 
                    key={index} 
                    className="p-2 text-sm hover:bg-accent cursor-pointer flex items-center"
                    onClick={() => handleSelectSuggestion(suggestion.text)}
                  >
                    <span>{suggestion.text}</span>
                    {suggestion.description && (
                      <span className="text-xs text-muted-foreground ml-2">{suggestion.description}</span>
                    )}
                  </div>
                ))}
              </div>
